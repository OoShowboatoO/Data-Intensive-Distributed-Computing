...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
  ...
  private static class MapClass extends
    ...
    private ArrayList<Integer> sourceNodes = new ArrayList<Integer>();

    @Override
    public void setup(Context context) throws IOException {

      String[] inputSources = context.getConfiguration().get("SOURCE_NODES").split(",");
      int srcLen = inputSources.length;

      for(int i = 0; i < srcLen ; i ++){
        sourceNodes.add(Integer.parseInt(inputSources[i]));
      }
    }
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      if (node.getAdjacencyList().size() > 0) {
        ...
        for (int i = 0; i < list.size(); i++) {
          ...
          // Current node is an dead end
          int srcLen = sourceNodes.size();
          float mass = node.getPageRank() - (float) StrictMath.log(srcLen);
          // Iterate over source nodes.
          for (int i = 0; i < srcLen; i++) {
            neighbor.set(sourceNodes.get(i));
            intermediateMass.setNodeId(sourceNodes.get(i));
            intermediateMass.setType(PageRankNode.Type.Mass);
            intermediateMass.setPageRank(mass);
  ...
  private static class ReduceClass extends
    ...
    public void reduce(IntWritable nid, Iterable<PageRankNode> iterable, Context context)
      ...
      if (structureReceived == 1) {
        // Combine phase2 into phase1 
        // Distribute missing mass, take care of random jump factor.
        int srcLen = sourceNodes.size();
        float p = mass;
        
        // Assume the node is not a source node
        float truePageRank = (float) Math.log(1.0f - ALPHA) + p;
        
        // Check if the node is a source node and update p accordingly
        for (int i = 0; i < srcLen; i++) {
          if (nid.get() == sourceNodes.get(i)) {
            float jump = (float) (Math.log(ALPHA) - Math.log(srcLen));
            // float link = (float) Math.log(1.0f - ALPHA) + sumLogProbs(p, (float) (Math.log(p) - Math.log(srcLen)));
            float link = (float) Math.log(1.0f - ALPHA) + p;
            
            truePageRank = sumLogProbs(jump, link);
          }
        }
  ...
  private void phase1(int i, int j, String basePath, int numNodes,
    ...
    job.getConfiguration().set("SOURCE_NODES", sources);
