...
object TrendingArrivals {
  ...
  def main(argv: Array[String]): Unit = {
    ...
    // Take online example as template
    // https://stackoverflow.com/questions/37347169/spark-mapwithstate-updated-states-output
    val updateState = StateSpec.function((key: String, currValue: Option[Int], state: State[Int]) => {          
        (currValue, state.getOption) match {
            case (Some(currState), Some(prevState)) => {
                state.update(currState)
                (key, currState, prevState)
            }
            case (Some(currState), None) => {
                state.update(currState)
                (key, currState, 0)
            }
            case _ => (key, 0, 0)
    ...
    val wc = stream.map(_.split(","))
      ...
      .mapWithState(updateState)
      .persist()

    val output = args.output()
    wc.foreachRDD((rdd, time) => {
      rdd.foreach(line =>{
        if (line._1.equals("goldman")) {
          if (line._2 >= 10 && line._2 >= line._3 * 2) {
            val currNum = line._2
            val precNum = line._3
            val timeStamp = time.milliseconds
            println(s"Number of arrivals to Goldman Sachs has doubled from $precNum to $currNum at $timeStamp!")
          }
        } else {
          if (line._2 >= 10 && line._2 >= line._3 * 2) {
            val currNum = line._2
            val precNum = line._3
            val timeStamp = time.milliseconds
            println(s"Number of arrivals to Citigroup has doubled from $precNum to $currNum at $timeStamp!")
          }
        }
      })
      rdd.map(line => (line._1, (line._2, time.milliseconds, line._3))) // (key, (curr, timestamp, prev))
      .saveAsTextFile(output + "/part-" + "%08d".format(time.milliseconds))
